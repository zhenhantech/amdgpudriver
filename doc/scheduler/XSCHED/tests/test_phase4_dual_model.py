#!/usr/bin/env python3
"""
Phase 4 Test: Dual Model Priority Scheduling
测试双模型优先级调度和 latency 保证

场景:
  - 高优先级任务: ResNet-18 在线推理 (10 reqs/sec)
  - 低优先级任务: ResNet-50 批处理 (连续运行)

目标:
  - 验证高优先级任务的 latency 保证
  - 验证低优先级任务不被饿死
  - 对比 XSched vs Native scheduler
"""
...
def high_priority_worker(duration, queue):
    ...
    模拟在线推理服务:
      - 10 reqs/sec (每 100ms 一个请求)
      - 每个请求处理 batch=1
      - 记录每个请求的延迟
    """
    try:
        print(f"[HIGH] Starting high priority task (ResNet-18)")
        print(f"[HIGH] Target: 10 reqs/sec, Duration: {duration}s")
...
def low_priority_worker(duration, queue):
    ...
    try:
        ...
        print(f"[LOW] Mode: Continuous inference, Duration: {duration}s")
        ...
        result = {
            ...
            'images_per_sec': (count * 8) / elapsed  # batch_size=8
        }
...
def run_test(duration=60, output_file=None):
    """
    运行双模型测试
    """
    print("=" * 70)
    print("Phase 4: Dual Model Priority Scheduling Test")
    print("=" * 70)
    print(f"\nTest configuration:")
    print(f"  Duration: {duration}s")
    print(f"  High priority: ResNet-18, 10 reqs/sec")
    print(f"  Low priority:  ResNet-50, continuous")
    print()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Phase 4 Dual Model Test')
